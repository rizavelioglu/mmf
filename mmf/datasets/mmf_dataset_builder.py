...
class MMFDatasetBuilder(BaseDatasetBuilder):
    ...
    def _split_dataset_from_train(self, dataset, dataset_type):
        if dataset_type in self.config.split_train.keys() or dataset_type == "train":
            ...
            indices = torch.randperm(dataset_length, generator=generator)[start:end]
    ...
    def _modify_dataset_config_for_split(self, config):
        for data_type in config.split_train:
            ...
            if config.use_images:
                with open_dict(config.images):
                    config.images[data_type] = copy.deepcopy(config.images.train)

            if config.use_features:
                with open_dict(config.features):
                    config.features[data_type] = copy.deepcopy(config.features.train)

            with open_dict(config.annotations):
                config.annotations[data_type] = copy.deepcopy(config.annotations.train)
        return config
